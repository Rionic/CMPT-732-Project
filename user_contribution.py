...
def get_percentiles(df, field):
    ...
    '''
    field_counts_by_user_with_NA = df.groupBy('OwnerUserId').agg(f.count('OwnerUserID').alias('count'))
    field_counts_by_user_with_NA.cache()
    total_field = field_counts_by_user_with_NA.count()
    field_counts_by_user = field_counts_by_user_with_NA.where(f.col('OwnerUserId') != 'NA')
    field_counts_by_user_sum = field_counts_by_user.groupBy('count').agg(f.sum('count').alias('total'))
    ...
    wdw = Window().orderBy(f.desc('count')).rowsBetween(Window.unboundedPreceding, Window.currentRow)
    ...
    percentile_breakpoints = field_counts_by_user.select(f.percentile_approx('count', percentiles)).head()[0]
    ...
    perc_df = spark.createDataFrame(percentile_scores, schema=['percentile', 'score'])
    perc_df = perc_df.groupBy('score').agg(f.max('percentile').alias('percentile'))
    res_df = perc_df.join(field_fracs, [perc_df['score'] == field_fracs['count']])
...
def main(input_q, input_a, input_t, output):
    q_df, a_df, _ = read_data(spark, input_q, input_a, input_t)
    q_perc = get_percentiles(q_df, 'questions')
    q_perc.show()
    q_perc.write.mode('overwrite').parquet(f'{output}/question_user_percentiles')
...
if __name__ == '__main__':
    ...
    main(input_q, input_a, input_t, output)